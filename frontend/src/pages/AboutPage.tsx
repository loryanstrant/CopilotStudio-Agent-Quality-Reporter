...
export default function AboutPage() {
  ...
  useEffect(() => {
    ...
    api.get<Freshness>("/reports/freshness").then(setFresh).catch(() => {});
  }, []);
    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
          What this tool does, how fresh the data is, and how it scores.
        </p>
      </div>
      ...
      <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
        <KpiCard label="Agents" value={fresh?.agents ?? "—"} />
        <KpiCard label="Environments" value={fresh?.environments ?? "—"} />
        <KpiCard label="Scans" value={fresh?.scans ?? "—"} />
        <KpiCard label="Findings" value={fresh?.findings ?? "—"} />
      </div>
      ...
      <div className="card p-6">
        ...
        <dl className="grid gap-3 text-sm sm:grid-cols-2">
          <Row label="Earliest scan" value={fmtDay(fresh?.earliest_scan ?? null)} />
          <Row label="Most recent scan" value={fmtDay(fresh?.latest_scan ?? null)} />
          <Row
            label="Last run"
      ...
      <div className="card p-6 text-sm leading-relaxed text-slate-600 dark:text-slate-300">
        ...
        <ul className="list-inside list-disc space-y-1">
          <li>
            Each agent starts at{" "}
            <span className="font-medium text-slate-900 dark:text-slate-100">100</span> and
            loses the configured weight of every failed rule; grades are A ≥ 90, B ≥ 75,
            C ≥ 60, D ≥ 40, F below.
          </li>
          <li>
          ...
          <li>
            ...
            connection outside Dataverse, so a service-principal scan never fails on its
            absence.
          </li>
        </ul>
      </div>
